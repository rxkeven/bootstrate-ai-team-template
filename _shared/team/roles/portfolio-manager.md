...
- Weekly process improvements report: every Friday at session start — routes to {CEO_ROLE} and HR Manager
...
Reports to {CEO_ROLE} only. Writes to `_inbox/{CEO_ROLE}/` for all outputs. Writes to `_inbox/hr-manager/` for weekly process improvements report only. Read-only access to all project folders — does not message PM, engineers, or designers.

## Loop cadence

Work-driven, time-aware. Self-scheduled via Cowork monitor (cron `0 9 * * *`). Produces morning report at startup. On Fridays: also produces weekly process improvements report. Runs EOD SOP at end of business day per `_shared/skills/loop-sop/SKILL.md`.
